...
void GenerateGrass(vec3 start, vec3 normal, vec3 direction, vec3 tangent, float uplength, float width, float bending, float wind, float seed){
    ...
    for(int i=0;i<5;i++){
        float mixfactor = float(i + 1.0) /5.0;
...
void GeometryGenerateGrass(){
    ...
    float dist = distance(gs_in[0].WorldPos, CameraPosition);
    dist = dist+1;
    dist = clamp((1.0/ (dist*dist))*18.0, 2, 18);
    int cnt = int(dist);
    ...
    for(int i=0;i<cnt;i++){
        ...
        //float height = texture(bumpMap, startPointTx).r*0.9;
        float height = 1.9;
        float width = rand(gs_in[2].TexCoord.yx-rd)*0.007 + 0.001; 
        
        GenerateGrass(startPoint, startNormal, normalize(rvec), startTangent, 1.9, width, 0.7, 0.3, rd);
    }
}
...
void GeometryProcessFlagDynamics(){
    ...
    for(int i=0;i<GEO_FLAG_YDIVISION;i++){
        ...
        p1 += n1 * snoise(vec4(p1*0.1 * noiseModifier, Time*timeModifier)) * (1.0-ymix1);
        p2 += n1 * snoise(vec4(p2*0.1 * noiseModifier, Time*timeModifier)) * (1.0-ymix2);
        p3 += n1 * snoise(vec4(p3*0.1 * noiseModifier, Time*timeModifier)) * (1.0-ymix1);
        p4 += n1 * snoise(vec4(p4*0.1 * noiseModifier, Time*timeModifier)) * (1.0-ymix2);
