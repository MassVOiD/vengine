...
vec3 ApplyLighting(vec3 albedo, vec3 position, vec3 normal, float roughness, float IOR){
    if(UseHBAO == 1) AOValue = AmbientOcclusion(position, normal, roughness);
...
#define MaterialTypeParallax 11
  ...
  //  else outSpecular = SpecularComponent;
    vec3 resultforward = ApplyLighting(color.xyz, wpos, rn, outRoughness, 1);
    outColor = vec4(resultforward, color.a);
