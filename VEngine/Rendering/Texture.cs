    ...
    internal static class ImageDDS
    {
        ...
        public static void LoadFromDisk(string filename, out uint texturehandle, out TextureTarget dimension)
        {
            ...
            try // Exceptions will be thrown if any Problem occurs while working on the file.
            {
                ...
                if(CheckFlag(pfFlags, (uint)eDDPF.FOURCC))
                    ...
                    {
                        case eFOURCC.DXT1:
                        _PixelInternalFormat = (PixelInternalFormat)OldGL.ExtTextureCompressionS3tc.CompressedRgbS3tcDxt1Ext;
                        _BytesPerBlock = 8;
                        _IsCompressed = true;
                        break;
                        //case eFOURCC.DXT2:
                        case eFOURCC.DXT3:
                        _PixelInternalFormat = (PixelInternalFormat)OldGL.ExtTextureCompressionS3tc.CompressedRgbaS3tcDxt3Ext;
                        _BytesPerBlock = 16;
                        _IsCompressed = true;
                        break;
                        //case eFOURCC.DXT4:
                        case eFOURCC.DXT5:
                        _PixelInternalFormat = (PixelInternalFormat)OldGL.ExtTextureCompressionS3tc.CompressedRgbaS3tcDxt5Ext;
                        _BytesPerBlock = 16;
                        _IsCompressed = true;
                        break;
                ...
                for(int Slices = 0; Slices < _Depth; Slices++)
                {
                    ...
                    for(int Level = 0; Level < _MipMapCount; Level++) // start at base image
                    {
                        ...
                        { // Note: there could be a potential problem with non-power-of-two cube maps
                            ...
                            GLError = GL.GetError();
                            if(TextureLoaderParameters.Verbose)
                                Trace.WriteLine("GL: " + GLError.ToString() + " Level: " + Level + " DXTn: " + ((compressed == 1) ? "Yes" : "No") + " Frmt:" + (OldGL.ExtTextureCompressionS3tc)internalformat + " " + width + "*" + height);
        ...
        public static void LoadFromByteArray(byte[] _RawDataFromFile, out uint texturehandle, out TextureTarget dimension)
        {
            ...
            catch(Exception e)
            {
                dimension = (TextureTarget)0;
                texturehandle = TextureLoaderParameters.OpenGLDefaultTexture;
                throw new ArgumentException("ERROR: Exception caught when attempting to load bytes \n" + e);
                // return; // failure
            }
            finally
            {
